...
The shared contract files are in `src/augury/schemas/`:

```powershell
python -c "from augury.schemas.experiment import *; p=ExperimentPlan(cycle_id=1,total_budget=100,primary_goal='Demos',allocations=[],exploration_budget_pct=0,exploitation_budget_pct=0,strategy_summary='test'); print(p.model_dump())"
```
...
```powershell
python -c "from augury.schemas.experiment import Allocation,Channel; Allocation(channel=Channel.META,experiment_id='x',current_budget=0,proposed_budget=-1,proposed_share=0,hypothesis='x',audience='x',message_angle='x',success_threshold=1,reason='x',evidence_used='x')"
```
...
The implementation is in `src/augury/constraints/budget.py`.

```powershell
python -c "from augury.services.intake import MockIntakeProvider; from augury.services.profiler import MockProfilerService; from augury.agents.strategist import StrategistAgent; from augury.schemas.ledger import HistoricalSummary; from augury.constraints.budget import validate_plan_constraints; b=MockIntakeProvider().get_founder_brief('ledger_ai'); p=MockProfilerService(); plan=StrategistAgent().plan_cycle(1,b,p.get_startup_profile('ledger_ai'),p.get_benchmark_priors('ledger_ai'),HistoricalSummary(startup_id='ledger_ai',total_cycles_completed=0,total_spend=0,total_outcomes=0,lifetime_cac=0),[]); print(validate_plan_constraints(plan,b))"
```
...
The Strategist is in `src/augury/agents/strategist.py`. Its local model is deterministic.
...
The baseline implementations are in `src/augury/baselines/allocators.py`:
...
The SQLite implementation is in `src/augury/ledger/sqlite.py`. The tests verify that plans, results, analyses, historical summaries, learnings, and failed hypotheses can be persisted and retrieved.

For manual inspection after a demo:

```powershell
python scripts/run_demo.py
python -c "import sqlite3; c=sqlite3.connect('data/demo_augury.db'); print(c.execute(\"select startup_id,cycle_id,channel,planned_budget,verdict from ledger_entries order by cycle_id\").fetchall())"
```
...
The approval interfaces are in `src/augury/approval/`. `AutoApprovalGate` approves test runs. `CLIApprovalGate` displays current spend, proposed spend, dollar movement, percentage movement, and rationale.

To test the interactive gate:

```powershell
python -c "from augury.approval.cli import CLIApprovalGate; print('CLIApprovalGate loaded:', CLIApprovalGate)"
```
...
The graph is built in `src/augury/graph/build.py`; nodes are in `src/augury/graph/nodes.py`.
...
```powershell
python -c "from augury.api.agentcore_app import entrypoint; r=entrypoint({'startup_id':'ledger_ai','cycle_id':1,'total_budget':2000,'auto_approve':True}); print(r['approval_status'],r['events_count'],len(r['verdicts']))"
```
...
The demo writes `data/demo_augury.db` and proves the full offline workflow.
...
The report compares Oracle, Equal Split, Founder Split, Greedy Winner, and Augury. It reports outcomes, CAC, regret, cycles to best channel, allocation distance, and agent quality metrics.
